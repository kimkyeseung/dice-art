...
export const Grid = memo(function Grid({ gridState, cellSize = 24, selectedDice, onCellUpdate, scale = 1, showMismatch = false, scrollContainerRef, rowOffset = 0, colOffset = 0 }: GridProps) {
  ...
  const lastPanPointRef = useRef<{ x: number; y: number } | null>(null); // 패닝용
  ...
  const fillCell = useCallback((row: number, col: number, skipFilled: boolean = false) => {
    if (selectedDice === null || selectedDice === 'pan') return;
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    // 패닝 모드 (preventDefault 불필요 - touch-none CSS로 처리)
    if (isPanMode) {
  ...
  }, [getCellFromPoint, cells, fillCell, resetCell, isPanMode]);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    // 패닝 모드 (preventDefault 불필요 - touch-none CSS로 처리)
    if (isPanMode && lastPanPointRef.current && scrollContainerRef?.current) {
  ...
  }, [isDragging, getCellFromPoint, fillCell, fillLine, cancelLongPress, isPanMode, scrollContainerRef]);
  ...
  // 터치 이벤트 기본 동작은 touch-none CSS로 처리됨
  // preventDefault 호출 시 passive 이벤트 경고 발생하므로 제거
  ...
  return (
    <div
      ref={gridRef}
      className={`inline-grid bg-neutral-300 gap-px p-px no-select touch-none ${isPanMode ? 'cursor-grab active:cursor-grabbing' : 'cursor-crosshair'}`}
      style={{
